...
router.route('/epics')
    ...
    .post(function (request, response) {
        var epic = new Epic();      // create a new instance of the Todo model

        epic.title = request.body.epic.title;  // set the todo's title (comes from the request)
        epic.isCompleted = request.body.epic.isCompleted; // set the todo's isCompleted property

        // save the todo and check for errors
        epic.save(function (error) {
            if (error) response.send(error);

            response.json({ message: 'Epic created!' });
        });
    })

    // get all the todos (accessed at GET http://localhost:8080/api/v1/todos)
    .get(function (request, response) {
        Epic.find(function (error, todos) {
            if (error) response.send(error);
            response.json({
                epics: epics
            });
